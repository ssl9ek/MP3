...
client.request("Observation?" + query, {
  ...
  function(ob) {
    ...
    if (weight[0].note && weight[0].note.length) {
        p.note = weight[0].note[weight[0].note.length - 1].text;
    }
    else {
        p.note = 'No Annotation'
    }
...
client.request(`MedicationRequest?patient=${client.patient.id}`, {
             resolveReferences: ["medicationCodeableConcept"],
             pageLimit: 0,
             flat: true
        })
        .then(
             function(meds) {
               meds.forEach(function(med) {displayMedication(med.medicationCodeableConcept.text)})
             }
        )
